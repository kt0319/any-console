...
`CLAUDE.md` は `@AGENTS.md` の1行のみでこのファイルを読み込みます。
...
コメントは既定で書かない (**SHOULD NOT**)。書くのは非自明な WHY（隠れた制約・特定バグの回避策・読者が驚く挙動・不変条件）がある時だけ。コードを読めば分かる WHAT を説明するコメントは書かない — 命名で意図を出す。

- 1〜2行で言えることを3行以上に広げない
- 他コンポーネント・他ファイルの実装詳細を延々と転記しない（参照だけに留める）
- 実装を変更したら、それを説明していた既存コメントも同時に見直す（矛盾・重複した記述を残さない）
- 同じ内容を複数箇所（JSDoc + 呼び出し元 + 変更履歴的な注記）で繰り返し説明しない
- 過去の実装や検討過程（「以前は〜だったが」「〜を試したがダメだった」等）は、今のコードの制約を理解する上で必要な場合のみ残す。単なる経緯メモは書かない
