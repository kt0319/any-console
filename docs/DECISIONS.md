...
- **Decision**: `list-panes -a` の一括取得に `#{pane_current_path}` を追加し、**ワークスペース未紐付けのセッションのみ**、cwd を登録済みワークスペースと最長前方一致で照合して `TMUX_WORKSPACE` を刻印する（`agent_watch._resolve_workspace`）。照合ロジックは Port Preview の `_match_workspace` と重複していたため `config.match_workspace_by_path` に一本化し、両者で共用する。既存の紐付け（明示操作・復元時・過去の自動紐付け）は上書きしない — `cd` で別のワークスペースへ移動しても付け替えない（復元時判定・明示紐付けと同じ「最初に確定したら維持」の意味論。付け替えたい場合は既存の UI 操作を使う）。
...
- **Consequences**: hooks を設定した Claude Code セッションは、ポーリング遅延なし・画面非依存で blocked / working / idle が判定される（blocked の将来のプッシュ通知もこのイベントを起点にできる）。設定していないセッション・他エージェントは従来どおり manifest / 画面差分で動き、体験は悪化しない。hooks は opt-in の設定手順（`~/.claude/settings.json`）が必要で、これは非侵襲方針との妥協 — ただしスクリプトは環境変数ゲートで any-console のセッション外では何もしない。運用で「hooks だけで困らない」ことが実測できた場合の manifest 層の縮小は、その時点で判断する（ADR 25 の教訓: 実測より先に常設機構を増減しない）。
