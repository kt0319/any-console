...
### 30. herdr の screen manifest をベンダリングして既知エージェントの blocked（承認待ち）検知を追加する

- **Status**: Accepted
- **Date**: 2026-08
- **Context**: agent_watch の状態は画面差分による working / idle の 2 値で、「Claude Code が許可を求めて止まっている」ことを検知できず、通知はジョブ定義ごとの notify_phrase 手動設定に頼っていた。承認待ちの検知は確認 UI の文言・レイアウトへの追従が必要で、自前でパターンを保守するのはコストが高い。一方 [ogulcancelik/herdr](https://github.com/ogulcancelik/herdr)（Apache-2.0）はエージェントごとの検知ルールを TOML（screen manifest）として保守しており、データとして流用できる。
- **Decision**: herdr の `claude.toml` / `codex.toml` を `api/agent_manifests/` へベンダリング（ライセンス同梱・リモート自動更新はせずリリース時に手動同期）し、評価エンジンのみ `api/screen_manifest.py` に Python で移植する（意味論は herdr の `manifest.rs` に合わせる: 定義順走査・優先度厳密大なり勝ち・`skip_state_update` は状態未確定）。エージェント特定は tmux `#{pane_current_command}` とマニフェスト id/aliases の照合（ネイティブバイナリ化した claude / codex はプロセス名で判別できる）。agent_watch のポーリングに `list-panes -a` 1 回を追加するだけで、周期・購読者ゼロ停止の構造は変えない（ADR 25 の教訓に準拠 — 常設機構の新設をしない）。判定が blocked のときだけ画面差分の状態を上書きし、UI はタブ / ピルの琥珀色点滅で表示する。`osc_progress` リージョンは tmux から取得できないため空とし該当ルールは自然に不一致（graceful degradation）。段階導入とし、blocked のプッシュ通知・他エージェントのマニフェスト追加は次段階で判断する。
- **Consequences**: 既知エージェントの承認・入力待ちがタブ一覧から一目で分かる。失敗モードは「blocked を見逃す」側に閉じる（マニフェスト破損はファイル単位で全捨てし検知なしへフォールバック。ルール単位のスキップはガードルール欠落で誤検知側に倒れるためしない）。ラッパー経由（node スクリプト等でプロセス名が claude/codex にならない）起動は検知対象外。upstream のルール書式が変わった場合は `tests/test_screen_manifest.py` のバンドル検証（未知リージョン・未知状態の検出）で顕在化する。
- **Alternatives considered**: **notify_phrase の拡充で代用** — ジョブごとの手動設定が前提で、確認 UI の文言変更に個人で追従し続けるのは非現実的。**herdr をプロセスとして併用** — herdr は自前の端末エミュレータを持つ多重化ツールであり、tmux を土台とする本アプリ（ADR 1）と二重構造になる。必要なのは検知ルールのデータだけ。**リモートマニフェスト自動更新（herdr 同等）** — 常時外部フェッチの常設機構は ADR 25 の教訓に反し、更新頻度もリリース同期で十分。**全状態（working / idle も）をマニフェスト判定に置き換え** — 画面差分方式は全エージェント・素のシェルにも効く汎用判定として実績があり、置き換えは hot path の大変更になる。blocked の上書きのみに絞る。
