...
- **Decision**: 認証済みセッションのみが呼べる `POST /auth/pairing/start` で、90秒だけ有効・一度使ったら即失効するペアリングID+トークンを発行する（`api/routers/pairing.py`、状態はプロセス内メモリのみ・ADR 1 の単一プロセス前提に準拠しディスクへは書かない）。QRコードは `https://<tailscaleホスト名>/pair/{id}?t=<token>` をエンコードするが、これは **Tailscale Serve が実際に稼働していると確認できた場合のみ**（`system._get_tailscale_serve_running()` を再利用）で、そうでなければ（tailscale未検出、またはインストール済みだがServe未設定でLAN/tailnet IPに直接bindしている構成）リクエスト自身の scheme/host/port にフォールバックする — MagicDNS名だけ解決できても実際にそこでリッスンしているとは限らないため、確認できない限りスキャン不能なURLをQRに埋め込まない。ホスト名解決は新設の `auth._resolve_tailscale_name()`（`tailscale status --json` の `Self.DNSName`）を使う。新デバイスは `GET /auth/pairing/{id}/status`（未認証で呼べる。発行元がQRモーダルのカウントダウン・自動クローズに使う）でポーリングし、`POST /auth/pairing/{id}/claim` でtokenを検証してログインを完了する。claim成功時は**既存の単一トークン認証と同じデバイスcookie発行ロジック**（`devices.register_device` + `routers/devices._set_device_cookies`）をそのまま再利用し、二重実装しない（`find_or_register_device` の同一UA再利用は使わない — 同一UAの2台を続けてペアリングすると後発のために先発のsecretを回転させ、先発デバイスを無言でログアウトさせてしまうため。claimは常に人間の明示操作であり、tailscale自動登録のような再利用の必要が無い）。tokenは「デバイス登録＋cookie発行が成功した後」に初めて破棄してclaimedへ倒す（登録が失敗した場合はtokenをまだ破棄前のエントリへロールバックし再試行可能にする）。claimed後のエントリはtombstoneとして残し、有効期限は縮めない — 発行元のポーリングがバックグラウンドタブ等で遅延しても「claimedを見損ねてexpired扱いになる」ことを防ぐ。start/status/claimはそれぞれ専用のレートリミッタ（`rate_limiter.py` の `_FixedWindowCounter` を再利用、id/tokenは24バイトの高エントロピーなランダム値なので総当たりは非現実的だが連打の二次防御として設ける）で絞る。claimはIPだけでなくpairing_idごとにも独立してカウントする — Tailscale Serve経由では全クライアントがloopback発に見えるため、IPのみだと無関係なpairing_idへの荒らしが正規の別ユーザーのpairingまで巻き添えでブロックしてしまう。フロントはQRを軽量ライブラリ `qrcode-generator`（ゼロ依存・MIT）でSVGとして描画し、vue-router非導入の方針を維持するため `/pair/{id}` のルーティングは `ui/utils/pairing.js` の純粋関数でURLをパースするだけの簡易な形にする（サーバ側の `GET /pair/{pairing_id}` はSPAシェル(index.html)をそのまま返すのみ）。既存のトークン手入力フローは変更せず、QRが使えない環境（カメラ無し等）向けのフォールバックとして残す。
