...
### 23. tmux の一時失敗は「不在」と区別し、クライアントはサーバ通知だけでセッションを削除しない

- **Status**: Accepted
- **Date**: 2026-07
- **Context**: tmux コマンド（`has-session` / `list-sessions` / `capture-pane`）はタイムアウト・OSError で一時的に失敗しうる（`_run_tmux_cmd` は None を返す）。特にセッション復元直後は一覧取得・履歴 capture・クライアントアタッチ・エージェント監視が同時に走り、失敗が起きやすい。この一時失敗を「セッション不在」と同一視するバグ類型が繰り返し混入した: (1) セッション一覧が空扱いになり全タブが消える（修正済み: 一覧 API は一時失敗で 500 を返す）、(2) エージェント状態が空扱いになる（修正済み: agent_watch）、(3) WS 接続時に「セッション不在」（close 1008）と誤通知 → フロントが tab:close → DELETE でセッション本体を削除、という連鎖で**生きている tmux セッションが読み込み直後に消える**。(3) はサーバの誤判定をクライアントが破壊的操作（DELETE）へ増幅する構造が本質だった。
- **Decision**: 2 つの原則を置く。**(a) tmux の存在確認は tri-state**: `has_tmux_session()` は True（存在）/ False（不在、tmux が正常応答）/ None（不明、コマンド失敗）を返し、None を False に潰さない。不明時は「不在」系の応答（WS close 1008 / HTTP 404 / 空配列）を返さず、リトライ可能な応答（WS close 1011 / HTTP 500）で閉じてクライアントの再接続バックオフ・再取得に委ねる。registry（`TERMINAL_SESSIONS`）の破棄やセッション再作成も行わない。旧 `tmux_session_exists()`（bool）は「不明も不在扱いで安全な箇所」（削除前の存在確認等、誤って何もしない側に倒れる用途）にのみ使う。**(b) クライアントはサーバのプッシュ通知だけを根拠にセッションを削除しない**: WS 1008（Session not found）を受けてもフロントは DELETE を送らずタブのローカル除去のみ行い（`tab:close` の `localOnly`）、真偽は 3 秒周期の同期ポーリング（`syncSessionsFromServer`）に確定させる — 誤判定ならタブが自動復活し、真に不在なら再追加されない。DELETE はユーザーの明示的なクローズ操作からのみ発行する。あわせて `closeTab` の pendingClose は try/finally で必ず解除し、同期時にサーバから消えたセッションの pendingClose を自動クリアして取り残しを自己回復させる。
- **Consequences**: tmux が一時的に固まっても、タブは "reconnecting" 表示で待って自動復帰し、生きているセッションが失われることはなくなる。誤判定時のタブ除去→ポーリング再追加で最大 3 秒程度タブが消えて見える瞬きは許容する（破壊的誤動作の回避を優先）。存在確認の呼び出し側は tri-state の分岐を意識する必要があり、bool に「簡略化」すると本バグ類型が再発する。真にセッションが消えたケースの後片付け（registry の残骸）はポーリングと次回解決時の失敗に委ねられる。
- **Alternatives considered**: **tmux コマンドのリトライで一時失敗を隠蔽** — 失敗の窓を狭めるだけで区別の問題は残り、復元直後の負荷をさらに増やす。**WS 1008 受信時にフロントから存在確認 API を叩いて真偽判定** — 専用の確認往復を足すより、既存の同期ポーリングに確定を委ねる方が経路が増えない。**サーバが不明時に接続を維持して内部リトライ** — WS ハンドラ内の待ち時間が延び、クライアントの既存バックオフ機構と二重になる。


---

### 24. セッション一覧はサーバ側スナップショット（TTL + last-known-good）から返す

- **Status**: Accepted（ADR 23 の原則を前提に、防御の置き場所を「呼び出し側ごと」から「読み取りの一本化」へ移す）
- **Date**: 2026-07
- **Context**: ADR 23 で「tmux の一時失敗を不在と区別する」原則を置いたが、その実装は呼び出し側ごとの点在防御（一覧 API の 500 化・WS close コードの分岐・メタデータ読み込みの tri-state / リトライ / 自己回復フラグ・フロントの再取得ヘルパーと workspace 自己回復ループ）として積み上がり、同型の防御を新しい呼び出し箇所へ再実装し続ける状態になっていた。真因は構造にある: `/terminal/sessions` が呼ばれるたびに list-sessions + セッション数ぶんの display-message / show-environment を実行し、複数クライアントが 3 秒間隔でこれをポーリングするため、subprocess 数がクライアント数 × セッション数に比例して膨らむ。さらに async ハンドラ内で blocking な `subprocess.run` を直接呼んでいたため、tmux が詰まるとイベントループごと止まり、他のリクエストや WS ハートビートまで巻き込んでタイムアウトの連鎖を自作していた。一時失敗は不運ではなく、負荷の自己増幅だった。
- **Decision**: 一覧の読み取りを `session_snapshot.py` に一本化する。(a) **1 コマンド化**: `list-sessions -F "#{session_name}\t#{session_created}"` で名前と作成時刻を一度に取得し、セッションごとの `display-message` を廃止。(b) **TTL（2 秒）**: TTL 内の読み取りは同じスナップショットを返し、tmux 呼び出しをクライアント数と切り離す。(c) **last-known-good**: 再構築に失敗しても最後に成功した一覧を返し続け、一度も成功していない場合のみ 500。「一時失敗がセッション消滅としてクライアントへ伝播する」経路そのものを塞ぐ。(d) **発見 = 登録**: 一覧に現れた未登録セッションはその場で registry へ登録し、メタデータ読み込み（show-environment）をセッションごと一度きりにする。メタデータが読めない間は不完全な登録・配信をせず、スナップショット構築ごと失敗させて last-known-good に倒す（`from_tmux` は None を返す。「workspace 無しで固定される」問題を部分リトライや自己回復フラグではなく all-or-nothing で防ぐ）。(e) **即時 invalidate**: セッションの作成・削除・属性変更は snapshot を即時無効化し、「作成直後のポーリングが古い一覧で作りたてのタブを閉じる」競合を防ぐ。(f) エンドポイントは sync def にして FastAPI の threadpool で実行し、イベントループから tmux subprocess を隔離する。フロントはこの一覧を信頼し、復元とポーリングで共通の冪等 reconcile（不足タブの追加・表示メタの追随・余剰タブの削除）を毎回適用するだけにする。表示メタ（workspace / ラベル / アイコン）はタブ生成時の焼き込みではなくポーリングごとにサーバ値へ追随させ、個別の再取得・自己回復ヘルパー（loadSessionsResponse / loadAllJobs / workspaceHealTargets）は削除した。
- **Consequences**: tmux subprocess は定常状態で TTL あたり 1 回（+ 新規発見時のメタデータ読み込み）になり、クライアント数・セッション数への比例が消える。一時失敗時もクライアントは常に直近の正しい一覧を受け取るため、タブの全消去・点滅・「素のターミナル化」のクラスが構造的に消える。代償として一覧の鮮度は最大 2 秒遅れる（変更操作は即時 invalidate で補う）。外部で直接 `tmux kill-session` された場合の反映も最大 TTL 分遅れるが許容する。スナップショットはプロセス内 state であり、single-process 前提（ADR 1）に依存する。
- **Alternatives considered**: **バックグラウンドタスクでの定期リフレッシュ** — 常駐タスクのライフサイクル管理が増える。オンデマンド + TTL は読み手がいない間 tmux を叩かない点でも有利。**メタデータの一次保存を tmux env から data/ の JSON へ移す** — show-environment 依存を消せるが、既存セッションからの移行・tmux との突き合わせ掃除という二重管理が増える。発見時一度きりの読み込みに削減できた現状では割に合わないため見送り（将来 show-environment 起因の問題が残るなら再検討）。**呼び出し側ごとの防御の継続** — 本 ADR の Context の通り、際限なく増える。
