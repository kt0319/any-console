...
- A random 32-character token is generated by `./any-console setup` (if
  `data/auth.json` does not exist yet) and printed once in the setup output.
- Signing in exchanges the token for a per-device cookie. Devices can be
  reviewed and revoked individually from **Settings → Auth**, and the
  token itself can be rotated there.
- A signed-in device can add another device via a QR code (**Settings →
  Auth → Trusted Devices → Add new device**) instead of retyping the token.
  The QR link is short-lived (90s) and single-use; it never widens access
  beyond the existing single-user model — it is device provisioning, not
  user invites. See `docs/DECISIONS.md` #28.
- There are no user accounts or roles. One token means full access. The one
  exception is scoped **API tokens** (Settings → Auth → API Tokens): they
  can only queue dispatch requests (`POST /dispatch`), but an approved
  dispatch still runs commands — treat them as secrets too.
- Request rate limiting and security headers are enabled by default.
- Authentication can be disabled — from the UI by turning off "Require
  token authentication" in **Settings → Auth** (this empties the token, so
  anyone with network access can use the console), or with
  `ANY_CONSOLE_DISABLE_AUTH=1` in the server process environment (in
  practice only `./any-console run` inherits your shell environment — a
  managed systemd/launchd service does not). Both are intended for
  disposable local development, never for a reachable deployment.

## If the token leaks

Assume full compromise of the OS user running any-console. Rotate the token
from **Settings → Auth** — note that rotating it does **not** invalidate
existing device cookies or API tokens, so also revoke all devices and API
tokens there. Do not just delete `data/auth.json` and restart: a server
without it runs with authentication disabled. Then audit the host: shell
history, tmux sessions, Git remotes/credentials, and `~/.ssh` reachable by
that user.
