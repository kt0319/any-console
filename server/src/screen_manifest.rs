...
fn gate_matches(gate: &Gate, text: &str, lower_text: &str) -> bool {
    ...
    if !gate.line_regex.is_empty() {
        let lines = crate::screen_regions::split_lines(text);
        if !gate
            .line_regex
            .iter()
...
pub fn evaluate_state(
    ...
    for rule in &manifest.rules {
        let text =
            crate::screen_regions::region_text(&rule.region, screen, osc_title, osc_progress);
            ...
            for rule in &manifest.rules {
                assert!(
                    SUPPORTED_REGIONS.contains(&rule.region.as_str())
                        || crate::screen_regions::region_count_re().is_match(&rule.region),
