...
async fn info_pills_normalize_order() {
    ...
    assert_eq!(got["order"].as_array().unwrap().len(), 10);
    ...
    assert_eq!(order.len(), 10, "未知キー除去・欠けキー補完");
}
...
/// jobDetached は PUT の保存形・GET 応答ともそのまま round-trip する。
/// v4 リネーム（jobDetachedTab → jobDetached）の過渡期ミラーは撤去済みで、
/// 旧キー jobDetachedTab は未知フィールドとして無視される（エラーにはならない）。
#[tokio::test]
async fn recent_jobs_detached_roundtrip() {
    let front = spawn_front().await;
    ...
    assert!(got["recent_jobs"][0].get("jobDetachedTab").is_none());
}
