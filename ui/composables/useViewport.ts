...
type FitCallback = (opts?: { scrollToBottom?: boolean }) => void;

let initialized = false;
const keyboardOpen = ref(false);
let keyboardCloseTimer: ReturnType<typeof setTimeout> | null = null;
let keyboardOpenTimer: ReturnType<typeof setTimeout> | null = null;
let fitDebounceTimer: ReturnType<typeof setTimeout> | null = null;
let orientationFitTimer: ReturnType<typeof setTimeout> | null = null;
let prevViewportHeightPx = 0;
let onFitCallback: FitCallback | null = null;
...
export function useViewport() {
  function initViewport(fitCallback: FitCallback) {
    ...
    document.addEventListener("touchend", (e) => {
      ...
      if ((e.target as Element | null)?.closest?.(".modal-overlay, .tab-menu-btn, .keyboard-bar")) return;
      const now = Date.now();
