...
const message = ref("");
const extraButton = ref<{ label: string, value: string, icon: string, desc: string } | null>(null);
const extra2Button = ref<{ label: string, value: string, icon: string, desc: string } | null>(null);
const okButton = ref<{ label: string, icon: string, danger: boolean } | null>(null);
const busy = ref(false);
const busyLabel = ref("");
const pending = createPendingPromise();
/** OK 押下後に実行する非同期処理。指定時はダイアログを閉じずに busy 表示にする。 */
let runOnOk: (() => Promise<void>) | null = null;
...
export function useConfirm() {
  function confirm(msg: string, opts: {
