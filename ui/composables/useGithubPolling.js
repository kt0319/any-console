import { useWorkspacePRs } from "./useWorkspacePRs.js";
...
// ポーリングの実装は useWorkspacePRs / useWorkspaceActions（それぞれ
// useWorkspaceResourcePoll.js の共通ファクトリ）のまま。
export function useGithubPolling() {
  ...
  /** @param {string} workspace 取得してからポーリングを開始する。 */
  function startGithubPolling(workspace) {
    fetchPRs(workspace);
    fetchRuns(workspace);
  ...
  function stopGithubPolling(workspace) {
