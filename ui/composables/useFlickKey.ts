...
type FlickKeyDef = { key: string };

type FlickKeyOpts = {
  accelerateRepeat?: boolean,
  onLongPress?: () => void,
  longPressGuard?: () => boolean,
  onFlick?: (key: FlickKeyDef, dx: number, dy: number) => boolean | void,
};
 ...
 */
export function attachFlickKey(
  el: HTMLElement,
  resolveKey: (dx: number, dy: number, threshold: number) => FlickKeyDef | null,
  sendKey: (keyDef: FlickKeyDef) => void,
  onTap: (() => void) | null = null,
  opts: FlickKeyOpts = {},
) {
  let startX = 0, startY = 0;
  let repeatTimer: ReturnType<typeof setTimeout> | null = null;
  let repeatingKey: FlickKeyDef | null = null;
  ...
  const scheduleRepeat = (key: FlickKeyDef, interval: number) => {
