...
const FIELDS = ["branch", "history", "prs", "actions", "changes", "pull_push", "devserver", "files", "add"];
...
export const useInfoPillConfigStore = defineStore("info-pill-config", () => {
  ...
  const pull_push = ref(true);
  const devserver = ref(true);
  const files = ref(true);
  const add = ref(true);
  const loaded = ref(false);

  const fieldRefs = { branch, history, prs, actions, changes, pull_push, devserver, files, add };
  ...
  return { branch, history, prs, actions, changes, pull_push, devserver, files, add, loaded, load, save };
});
