...
const FIELDS = ["branch", "history", "prs", "changes", "pull_push", "devserver", "files", "add"];
...
export const useInfoPillConfigStore = defineStore("info-pill-config", () => {
  ...
  const fieldRefs = { branch, history, prs, changes, pull_push, devserver, files, add };
  ...
  return { branch, history, prs, changes, pull_push, devserver, files, add, loaded, load, save };
});
