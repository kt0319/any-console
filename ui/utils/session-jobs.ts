...
/**
 * allJobs が空のままジョブセッションを復元すると、タブのジョブアイコンが mdi-play に
 * フォールバックし以後リロードまで直らない（tab.icon は焼き込み）。レスポンス全体では
 * なく、セッションが参照するワークスペース単位でジョブ定義が空なら一時失敗とみなして
 * 再取得すべき（他ワークスペース分は届いているのに特定ワークスペースだけ空になるケースがある）。
 *
