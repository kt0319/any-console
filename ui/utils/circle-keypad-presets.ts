...
// symbol はサークルキーパッド上（ドラッグ中のリング表示）で使う短縮表記。
// 設定画面のドロップダウンでは読みやすい label をそのまま使う。
export const CIRCLE_KEYPAD_MODIFIER_OPTIONS = [
  { id: "none",           label: "—",              symbol: "",    ctrl: false, shift: false, alt: false },
  { id: "shift",          label: "Shift",          symbol: "⇧",   ctrl: false, shift: true,  alt: false },
  { id: "ctrl",           label: "Ctrl",           symbol: "⌃",   ctrl: true,  shift: false, alt: false },
  { id: "alt",            label: "Alt",            symbol: "⌥",   ctrl: false, shift: false, alt: true },
  { id: "ctrl_shift",     label: "Ctrl+Shift",     symbol: "⌃⇧",  ctrl: true,  shift: true,  alt: false },
  { id: "ctrl_alt",       label: "Ctrl+Alt",       symbol: "⌃⌥",  ctrl: true,  shift: false, alt: true },
  { id: "shift_alt",      label: "Shift+Alt",      symbol: "⇧⌥",  ctrl: false, shift: true,  alt: true },
  { id: "ctrl_shift_alt", label: "Ctrl+Shift+Alt", symbol: "⌃⇧⌥", ctrl: true,  shift: true,  alt: true },
];
...
export const CIRCLE_KEYPAD_SPECIAL_PRESETS = [
  ...
  { id: "wheelup",   label: "Wheel Up",   action: "terminal:wheelUp" },
  { id: "wheeldown", label: "Wheel Down", action: "terminal:wheelDown" },
  { id: "scrollup",  label: "Scroll Up",  action: "terminal:scrollUp" },
  { id: "scrolldown", label: "Scroll Down", action: "terminal:scrollDown" },
...
// 8 方向のラベル（N から時計回り）。
export const CIRCLE_KEYPAD_DIRECTION_LABELS = ["↑", "↗", "→", "↘", "↓", "↙", "←", "↖"];

// デフォルトの 8 方向キー（N から時計回り）。modifier は CIRCLE_KEYPAD_MODIFIER_OPTIONS の id。
export const DEFAULT_CIRCLE_KEYPAD_KEY_DEFS = [
...
export function circleKeypadKeyLabel(modifierId: string | null | undefined, keyId: string | null | undefined) {
  ...
  return mod.symbol ? `${mod.symbol}${keyLabel}` : keyLabel;
}

export function defaultKeyDefs() {
  return DEFAULT_CIRCLE_KEYPAD_KEY_DEFS.map(({ modifier, key }) => {
    ...
    return {
      ...
      shift: mod.shift,
      alt: mod.alt,
      label: circleKeypadKeyLabel(modifier, key),
    };
  });
}

export function defaultSpecialDefs() {
  return DEFAULT_CIRCLE_KEYPAD_SPECIALS.map((id) => {
