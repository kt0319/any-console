...
def test_touch_device_calls_verify_and_touch():
    with patch("api.routers.migration_bridge.verify_and_touch_device") as touch:
        res = loopback_client.post(
            "/internal/touch-device", json={"device_id": "dev_1", "raw_secret": "raw"}
        )
    assert res.status_code == 200
    assert res.json() == {"status": "ok"}
    touch.assert_called_once_with("dev_1", "raw")


def test_touch_device_rejects_non_loopback():
    with patch("api.routers.migration_bridge.verify_and_touch_device") as touch:
