...
import { normalizeLayoutPrefs, resolveTabPosition, resolveKeyboardBarVisible, DEFAULT_LAYOUT_PREFS } from "../../ui/utils/layout-prefs.ts";
...
describe("normalizeLayoutPrefs", () => {
  ...
  it("不正な値は既定値にフォールバックする", () => {
    expect(normalizeLayoutPrefs({ narrowTabPosition: "left", wideKeyboardBar: "yes" })).toEqual(DEFAULT_LAYOUT_PREFS);
  });
...
describe("resolveTabPosition / resolveKeyboardBarVisible", () => {
  ...
  it("狭い/広いそれぞれ個別に設定した値を反映する", () => {
    const prefs = { narrowTabPosition: "top", wideTabPosition: "top", narrowKeyboardBar: false, wideKeyboardBar: true };
