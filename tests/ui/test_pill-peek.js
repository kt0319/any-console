...
describe("trailingItemsSignature", () => {
  it("key -> icon|text のMapを作る", () => {
    const items = [{ key: "branch", icon: "mdi-source-branch", text: "main" }];
    const sig = trailingItemsSignature(items);
    expect(sig.get("branch")).toBe("mdi-source-branch|main");
  });
...
describe("findChangedTrailingItem", () => {
  it("新規追加された項目を検出する", () => {
    ...
    const items = [{ key: "push", icon: "mdi-arrow-up", text: "Push 1" }];
    expect(findChangedTrailingItem(items, prev)).toEqual(items[0]);
  });

  it("既存項目のテキストが変わったら検出する", () => {
    const prev = trailingItemsSignature([{ key: "push", icon: "mdi-arrow-up", text: "Push 1" }]);
    const items = [{ key: "push", icon: "mdi-arrow-up", text: "Push 2" }];
    expect(findChangedTrailingItem(items, prev)).toEqual(items[0]);
  });
  ...
  it("複数変化していれば並び順で最初の項目を返す", () => {
    const prev = trailingItemsSignature([
      { key: "branch", icon: "mdi-source-branch", text: "main" },
      { key: "push", icon: "mdi-arrow-up", text: "Push 1" },
    ]);
    const items = [
      { key: "branch", icon: "mdi-source-branch", text: "feature/x" },
      { key: "push", icon: "mdi-arrow-up", text: "Push 2" },
    ];
    expect(findChangedTrailingItem(items, prev)).toEqual(items[0]);
  });
